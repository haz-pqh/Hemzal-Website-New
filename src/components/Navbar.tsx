...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-40 select-none">
      ...
      >
        {/* Expanded max-width from max-w-7xl (1280px) to max-w-[1600px] with increased side padding */}
        <div className="max-w-[1600px] mx-auto px-4 sm:px-8 lg:px-12 flex items-center justify-between gap-6">
          ...
          <div className="hidden lg:flex items-center gap-2 xl:gap-4">
            {navLinks.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="relative px-4 py-2 rounded-xl text-sm font-semibold text-neutral-300 hover:text-white hover:bg-white/5 transition-all flex items-center gap-1.5 group"
              >
          ...
          <div className="flex items-center gap-3 sm:gap-4 shrink-0">
            ...
            <button
              ...
              className="hidden md:inline-flex items-center gap-2 text-xs font-semibold text-neutral-300 hover:text-white px-4 py-2.5 rounded-xl border border-white/10 hover:border-[#FDB913]/40 bg-white/5 hover:bg-white/10 transition-all cursor-pointer shadow-sm"
            ...
            <button
              ...
              className="relative flex items-center justify-center gap-2 bg-[#1b1b1e] hover:bg-[#25252a] text-white px-4 py-2.5 rounded-xl border border-white/15 hover:border-[#FDB913]/50 transition-all shadow-md cursor-pointer group shrink-0"
            ...
            <a
              href="#menu"
              className="hidden sm:inline-flex items-center justify-center gap-1.5 bg-gradient-to-r from-[#E31E24] to-[#C1121F] hover:from-[#f52b32] hover:to-[#e31e24] active:scale-95 text-white font-bold text-xs tracking-wider uppercase px-5 sm:px-6 py-2.5 rounded-xl shadow-lg shadow-[#E31E24]/30 hover:shadow-[#E31E24]/50 transition-all cursor-pointer shrink-0"
            >
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden bg-[#121215]/98 backdrop-blur-xl border-b border-white/10 px-6 py-5 space-y-4 shadow-2xl animate-in slide-in-from-top-3 duration-200 max-h-[calc(100vh-100px)] overflow-y-auto">
